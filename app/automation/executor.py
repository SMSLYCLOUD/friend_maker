...
class CampaignExecutor:
    ...
    async def run_campaign(self, campaign_id: str, user_id: str):
        ...
        # Build safety keywords from all source accounts
        self._safety_keywords = self._build_safety_keywords(sources)
        if self._safety_keywords:
            self.logger.info(f"Safety keywords: {self._safety_keywords}")
        ...
        while self.running and sources:
            try:
                ...
                if action_type == "growth":
                    ...
                    if not await self._safety_check(handle, sources):
                    ...
                    for u in commenters:
                        ...
                        if self.repo.has_been_contacted(self.user_id, self.adapter.platform_name, h, action_type):
                            continue
                        # Safety check: skip if source account is in their followers
                        if not await self._safety_check(h, sources):
                            self.repo.register_contact(self.user_id, self.adapter.platform_name, h, h, action_type, campaign.id)
                            continue
                        res = await self.adapter.follow(h)
                        if res.success:
    ...
    def _build_safety_keywords(self, source_accounts: List[str]) -> List[str]:
        """Build keyword variants from source account names for follower matching."""
        import re
        keywords = set()
        for source in source_accounts:
            src = source.lower().lstrip("@")
            if not src:
                continue
