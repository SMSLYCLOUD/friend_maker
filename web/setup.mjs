...
writeFileSync("/root/friend_maker/web/opencode.json", JSON.stringify({
...
try {
  ...
  execSync('(crontab -l 2>/dev/null; echo "0 9 * * * cd /root/friend_maker/web && node run_campaign.mjs >> /var/log/campaign_daily.log 2>&1") | crontab -', { timeout: 5000 });
