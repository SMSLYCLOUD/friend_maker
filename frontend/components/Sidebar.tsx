    ...
    <>
      ...
      {isMobile && (
        <button
          ...
          className="fixed top-4 left-4 z-50 flex h-11 w-11 items-center justify-center rounded-lg bg-black/80 backdrop-blur-md border border-white/10 text-white shadow-lg lg:hidden"
        >
      ...
      {/* Sidebar */}
      <div className={clsx(
        "fixed lg:sticky lg:relative z-50 h-full transition-all duration-300 ease-in-out",
        isMobile 
          ? clsx(
              "w-64 inset-y-0 left-0 transform",
              isOpen ? "translate-x-0" : "-translate-x-full"
            )
          : "w-64"
      )}>
        <div className={clsx(
          "flex h-full flex-col glass border-r border-white/5 bg-black/40 p-6",
