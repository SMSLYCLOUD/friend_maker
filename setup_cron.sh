...
cat > /root/friend_maker/opencode.json << 'CONFIG'
{
  "permission": {
    "bash": { "*": "allow" }
  }
...
echo "[1/3] Config created at /root/friend_maker/opencode.json"
...
(crontab -l 2>/dev/null; echo "0 9 * * * cd /root/friend_maker && node run_campaign.mjs >> /var/log/campaign_daily.log 2>&1") | crontab -
echo "[3/3] Cron job added — runs daily at 09:00"

echo ""
echo "=== Next, test with this Telegram command ==="
echo ""
echo "exec cd /root/friend_maker && node run_campaign.mjs"
echo ""
