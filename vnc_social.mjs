    ...
    try {
      ...
      if (url.pathname === "/capture") {
        ...
        try {
          ...
          // Start monitoring for login in background
...
novncProcess = start("bash", ["/opt/noVNC/utils/novnc_proxy", "--vnc", `localhost:${VNC_PORT}`, "--listen", String(NOVNC_PORT), "--web", "/opt/noVNC"]);
await wait(3000);
...
console.log(`VNC:  http://${hostIp}:${NOVNC_PORT}/vnc.html`);
...
try {
  ...
  console.error("[WARN] Failed to load blank page:", error.message);
}
